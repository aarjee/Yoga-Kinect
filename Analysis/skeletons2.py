...
def getColorFrame(path,n):
    cap = cv2.VideoCapture(path+'/color.avi')
    i = 0
    while(cap.isOpened()):
        ret, frame  = cap.read()
        if(i==n):
            frame = frame[0:1000,0:1400]
            frame1 = frame.reshape((1000, 1400, 3))
            frame2 = cv2.cvtColor(frame1, cv2.COLOR_RGBA2RGB)
            return frame2

            break
        i+=1
    empty = np.zeros((1000,1400,3), dtype = np.uint8)
...
def draw_skeleton(path):
    ...
    fps = 7
    capSize = (1000, 1400) # this is the size of my source video
...
if __name__ == '__main__':
    '''for x in os.walk('YogaKinect'):
        ...
        draw_skeleton(x[0])'''
    if(len(sys.argv)!=2):
    	sys.exit('Usage:\nInput:<directory containing the joints.csv sheet>\nCreates smooth.avi video of skeleton in the entered directory')
    path = sys.argv[1]
    draw_skeleton(path)
